...
const ImgTextAnimation = ({ imgSrc, text, title1, title2 }) => {
    ...
    const blurAmount = Math.min(scrollY / 10, 5); // Max blur of 10px
    const opacityAmount = Math.max(1 - scrollY / 1000, 0.5); // Min opacity of 0.5

    return (
        <div
            className="flex justify-around items-center lg:p-8 p-4  w-full  overflow-hidden bg-gray-900 rounded-3xl flex-wrap-reverse text-center"
            ref={containerRef}
            style={{
        ...
        >
            ...
            <div
                ref={textRef}
                className="text-white text-xl font-semibold m-6 flex flex-col flex-wrap lg:w-1/2"
                style={{ opacity: 0 }} // Initially hidden
            >
                <h1 className="lg:text-5xl text-lg  font-bold text-white lg:mb-0 -mb-2 text-left">{title1}</h1>
...
export default ImgTextAnimation;
